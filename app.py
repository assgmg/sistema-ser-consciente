...
def init_db():
  ...
  # Atendimentos / Lançamentos (Com trava de consolidação para conciliação bancária)
  cursor.execute("""
        CREATE TABLE IF NOT EXISTS atendimentos (
            ...
            recepcionista TEXT,
            status TEXT DEFAULT 'Ativo', -- Ativo, Cancelado, Atualizado
            consolidado INTEGER DEFAULT 0 -- 0 = Aberto/Editável, 1 = Consolidado/Imutável para Banco
        )
    """)
...
if menu == "Registro de Atendimentos":
  ...
  if not profissionais:
    st.warning("Nenhum profissional ativo cadastrado.")
  else:
  ...
  st.write(
      "Registre novos pacotes vendidos ou dê baixa nas sessões consumidas sem"
      " afetar o caixa operacional diário."
  )
  # Seção simplificada para controle de pacotes
  ...
  with st.form("form_pacote"):
    ...
    if btn_cad_pacote:
      if p_pac and val_pac > 0:
        ...
        conn.close()
        st.success(
            "Pacote registrado com sucesso! O consumo será abatido conforme os"
            " atendimentos."
        )
      else:
        st.error("Preencha todos os campos do pacote corretamente.")

elif menu == "Fechamento de Caixa e Fatura":
  st.header("💰 Fechamento de Caixa e Emissão de Fatura para Cobrança")
  st.write(
      "Visualize os atendimentos do dia, calcule o repasse e gere a fatura em"
      " HTML/PDF para envio e cobrança."
  )
  ...
  if df_caixa.empty:
    ...
    if st.button("Gerar Fatura em Documento Pronto para Impressão"):
      ...
      # HTML da Fatura renderizado de forma limpa para impressão / PDF
      html_fatura = f"""
...
# --- 4. MÓDULOS GERENCIAIS (ADMINISTRAÇÃO) ---
  ...
  st.header(
      "🔍 Auditoria, Correção de Status e Travamento de Lançamentos"
  )
  st.markdown(
      "Aqui você pode atualizar status de lançamentos ou consolidá-los para"
      " garantir a conciliação com o extrato do banco."
  )
  ...
  if not df_aud.empty:
    ...
    if st.button("Atualizar Lançamento com Segurança"):
      ...
      st.success(
          f"Lançamento ID {at_id} atualizado com sucesso sem corromper a base!"
      )
  ...
  st.dataframe(
      pd.DataFrame(
          ...
          columns=["ID", "Nome", "Tipo de Contrato", "Ativo"],
      ),
  ...
  with st.form("form_desp"):
    ...
    if st.form_submit_button("Lançar Despesa"):
      ...
      st.success("Despesa registrada para projeção de fluxo de caixa.")

elif menu == "Conciliação Bancária":
  st.header("🔄 Conciliação Bancária Automatizada")
  st.markdown(
      "Faça o upload do extrato (OFX/CSV) para cruzar com os lançamentos"
      " consolidados do sistema."
  )
  st.file_uploader("Enviar Extrato Bancário", type=["ofx", "csv", "txt"])
